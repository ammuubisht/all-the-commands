...
export default function CommandsPage({ params }: { params: { tool: string } }) {
  const commands = [
    {
      name: "git init",
    ...
    },
    {
      ...
      description:
        "Restore the specified file to its state in the last commit.",
      category: "Using Branches",
  ...
  const groupByCategory = (commands: any[]) => {
  ...
  return (
    <main className="min-h-screen p-24">
      <h1 className="scroll-m-20 text-4xl font-extrabold tracking-tight lg:text-5xl mb-32">
        {params.tool} Commands
      </h1>
      {Object.keys(commandsByCategory).map((category, index) => {
        return (
          <Card className="w-full my-4" key={index}>
            <CardHeader>
            ...
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {commandsByCategory[category].map(
                        ...
                        <TableRow key={index}>
                          <TableCell className="w-2/4">
                            <div className="font-medium flex items-center gap-2">
                              ...
                              <CopyButton commandName={command.name} />
                            </div>
                          </TableCell>
